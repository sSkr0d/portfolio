...
export const Vortex = (props: VortexProps) => {
  ...
  const yOff = 0.0012;
  const zOff = 0.0006;
  const swirlStrength = props.swirlStrength ?? 0.9;
  ...
  const draw = (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) => {
    ...
    // clear canvas; do not draw a solid background so page body color shows through
  ...
  const updateParticle = (i: number, ctx: CanvasRenderingContext2D) => {
    ...
    // base noise-driven direction
    n = noise3D(x * xOff, y * yOff, tick * zOff) * noiseSteps * TAU;

    // add a subtle swirl/curl component perpendicular to the noise angle
    const curl = noise3D(x * (xOff * 0.5), y * (yOff * 0.5), tick * (zOff * 0.8));
    const curlAngle = n + HALF_PI * curl;
    const curlX = Math.cos(curlAngle);
    const curlY = Math.sin(curlAngle);

    // combine noise direction with curl to create organic, unpredictable motion
    vx = lerp(particleProps[i3], Math.cos(n) + curlX * swirlStrength, 0.5);
    vy = lerp(particleProps[i4], Math.sin(n) + curlY * swirlStrength, 0.5);
